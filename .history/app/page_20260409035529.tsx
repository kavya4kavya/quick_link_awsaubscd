...
export default function EventSocialLinksPage() {
  ...
  const getIconContainerClass = (link: (typeof links)[number]) => {
    if (link.logo) {
      return "relative flex h-10 w-20 items-center justify-center overflow-hidden rounded-xl border border-white/25";
    }

    return "relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl bg-linear-to-br from-purple-500/30 to-fuchsia-400/20 text-xl border border-white/10";
  };

  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#7c3aed_0%,#1e1b4b_35%,#020617_100%)] text-white flex items-center justify-center px-6 py-10">
